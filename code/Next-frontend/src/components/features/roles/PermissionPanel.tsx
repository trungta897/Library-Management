...
export default function PermissionPanel({ role, onTogglePermission }: PermissionPanelProps) {
    return (
        <div className="flex h-full flex-col overflow-hidden rounded-xl border border-outline-variant/30 bg-white shadow-sm">
            <div className="flex items-center justify-between border-b border-outline-variant/20 bg-white px-6 py-5">
                <div>
                    <p className="mb-1 text-xs font-semibold uppercase tracking-widest text-secondary">{UI_TEXT.ROLES.EDITING_ROLE}</p>
                ...
                <div className="flex items-center gap-3">
                    <button type="button" className="rounded-lg border border-primary px-4 py-2 text-primary transition hover:bg-primary hover:text-white">
                        {UI_TEXT.ROLES.DISCARD_CHANGES}
                    </button>

                    <button type="button" className="rounded-lg bg-primary px-4 py-2 text-white transition hover:opacity-90">
