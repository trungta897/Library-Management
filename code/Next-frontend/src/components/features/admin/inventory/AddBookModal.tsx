    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                ...
                // Reset form or set from initialData after reference data is loaded
                setTitle(initialData?.title || "");
                setIsbn(initialData?.isbn || "");

                // Map authors
                if (initialData?.authors && initialData.authors.length > 0) {
                    const mappedAuthors = initialData.authors.map((authorName) => {
                        const existing = authData.find((a: Author) => a.name.toLowerCase() === authorName.toLowerCase());
                        if (existing) {
                            return { label: existing.name, value: existing.id.toString(), __isNew__: false };
                        }
                        return { label: authorName, value: authorName, __isNew__: true };
                    });
                ...
                if (initialData?.category) {
                    const existingCat = catData.find((c: Category) => c.name.toLowerCase() === initialData.category?.toLowerCase());
                    if (existingCat) {
                        setSelectedCategories([{ label: existingCat.name, value: existingCat.id.toString(), __isNew__: false }]);
                    } else {
                        setSelectedCategories([{ label: initialData.category, value: initialData.category, __isNew__: true }]);
                    }
                } else {
                ...
                setPublicationDate("");
                setPages(initialData?.pageCount || "");
                setDepositPrice("");
                setError(null);
            } catch (err) {
