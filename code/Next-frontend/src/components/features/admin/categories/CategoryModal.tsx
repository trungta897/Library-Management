...
export default function CategoryModal({ category, isOpen, onClose, onSuccess }: CategoryModalProps) {
    ...
    const textUI = ADMIN_CATEGORY_MANAGEMENT.MODAL;
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink-950/40 p-4 backdrop-blur-sm">
            <div className="relative flex w-full max-w-md flex-col overflow-hidden rounded-2xl bg-white shadow-xl">
                <div className="flex shrink-0 items-center justify-between border-b border-surface-container-high px-6 py-4">
                    <h2 className="text-[18px] font-semibold text-ink-950">{category ? textUI.TITLE_EDIT : textUI.TITLE_ADD}</h2>
                ...
                <div className="p-6">
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div className="space-y-1.5">
                            <label className="text-[13px] font-medium text-on-surface-variant">{textUI.LABEL_DESC}</label>
                            <textarea
                                value={description}
