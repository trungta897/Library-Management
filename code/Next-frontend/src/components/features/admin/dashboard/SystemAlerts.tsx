...
export default function SystemAlerts() {
    return (
        <div className="shadow-card rounded-xl border border-ink-950/[0.06] bg-white p-5">
            <h2 className="text-[16px] font-semibold text-ink-950">{UI_TEXT.ADMIN_LAYOUT.ALERTS.HEADING}</h2>

            <div className="mt-3.5 space-y-2.5">
                {ALERTS.map((alert) => {
                    const styles = TONE_MAP[alert.tone];
                    return (
                        <div key={alert.id} className={`flex gap-2.5 rounded-lg border ${styles.border} ${styles.bg} px-3.5 py-3`}>
                            ...
                            <div className="min-w-0">
                                ...
                                <p className="mt-0.5 text-[12.5px] leading-snug text-ink-950/55">{alert.body}</p>
                            </div>
                        </div>
                    );
                })}
