...
function ActionButtons({ status, onStatusUpdate }: { status: BorrowStatus; onStatusUpdate: (newStatus: BorrowStatus) => void }) {
    return (
        <div className="flex justify-end gap-1 opacity-0 transition-opacity group-hover:opacity-100">
            ...
            <button className="rounded p-1.5 text-secondary transition-colors hover:bg-secondary-fixed/50" title={T.BTN_VIEW}>
                <Eye size={20} />
            </button>
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-left text-sm">
                    ...
                    <thead className="border-b border-outline-variant/30 bg-surface-container-low font-label-caps text-label-caps text-on-surface-variant">
                        <tr>
                            <th className="px-6 py-4 font-medium">{T.COL_MEMBER}</th>
                            <th className="px-6 py-4 font-medium">{T.COL_BOOK}</th>
                            <th className="px-6 py-4 font-medium">{T.COL_BORROW_DATE}</th>
                            <th className="px-6 py-4 font-medium">{T.COL_DUE_DATE}</th>
                            <th className="px-6 py-4 font-medium">{T.COL_STATUS}</th>
                            <th className="px-6 py-4 text-right font-medium">{T.COL_ACTIONS}</th>
                        </tr>
                    </thead>
