...
import { FAVORITE_BOOKS_MOCK } from "@/mocks/favorites";
...
function MyBooksContent() {
    ...
    const displayedBooks = FAVORITE_BOOKS_MOCK.slice(0, limit);
    const hasMore = limit < FAVORITE_BOOKS_MOCK.length;
    ...
    return (
        <main className="mx-auto flex w-full max-w-container-max flex-grow flex-col gap-xl px-md py-xl md:px-lg">
                    ...
                    >
                        ...
                        <div className="relative aspect-[3/4] w-full overflow-hidden bg-surface-container-low">
                            ...
                            <div className="absolute inset-0 flex items-center justify-center bg-surface-tint/20 opacity-0 backdrop-blur-[2px] transition-opacity duration-300 group-hover:opacity-100">
                                <button className="rounded-lg bg-primary px-lg py-sm font-title-md text-title-md text-on-primary shadow-md transition-colors hover:bg-primary-container hover:text-on-primary-container">
                        ...
                        <div className="flex flex-grow flex-col bg-surface-container-lowest p-md">
                            <div className="mb-sm flex items-start justify-between gap-sm">
                                <div>
                                    <h3 className="line-clamp-2 font-title-md text-title-md text-on-surface">{book.title}</h3>
                                    <p className="mt-xs font-body-sm text-body-sm text-on-surface-variant">{book.author}</p>
                                </div>
                                <button
                                    className="group/btn flex-shrink-0 rounded-full p-xs text-error transition-colors hover:bg-error-container"
                                    title={UI_TEXT.FAVORITE.REMOVE_TITLE}
                                >
                                    <MaterialIcon
                                        name="bookmark_add"
                                        className="transition-transform group-active/btn:scale-110"
                            ...
                            <div className="mt-auto flex items-center justify-between border-t border-outline-variant/20 pt-md">
                                <span className="font-label-caps text-label-caps uppercase tracking-wider text-outline">{book.category}</span>
                                <button
                                    className="flex items-center gap-xs text-outline-variant transition-colors hover:text-error"
                                    title={UI_TEXT.FAVORITE.REMOVE_TITLE}
                                >
            ...
            {hasMore && FAVORITE_BOOKS_MOCK.length > 0 && (
            ...
            {FAVORITE_BOOKS_MOCK.length === 0 && (
                <div className="flex flex-col items-center justify-center py-xl text-center">
                    ...
                    <button className="mt-lg rounded-lg bg-primary px-xl py-sm font-title-md text-title-md text-on-primary shadow-[0_4px_12px_rgba(0,0,0,0.05)] transition-colors hover:bg-primary-container hover:text-on-primary-container">
