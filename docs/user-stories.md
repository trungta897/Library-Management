...
\- Nút "Mượn sách không cần tài khoản" chuyển đến form đặt mượn dành cho Guest (theo US-37)
...
**AC-04: Guest xem sách hết kho — không thể Đặt giữ sách**
...
- Không hiển thị nút "Đặt giữ sách" cho Guest (tính năng Đặt giữ sách yêu cầu đăng nhập - US-09)
- Hiển thị gợi ý "Đăng nhập để Đặt giữ sách và nhận thông báo khi sách có sẵn"
...
\- Nếu hết sách: hiển thị "Sách hiện không có sẵn", gợi ý "Đặt giữ sách"
...
\- Sách vẫn còn trong kho khả dụng (Tổng tồn kho - Số lượng đang được đặt giữ > 0)
...
\- Customer submit sau nhận thông báo "Sách vừa được người khác mượn. Bạn có muốn Đặt giữ sách?"
...
- Hiển thị nút "Đặt giữ sách" (reservation)
...
- Customer submit sau nhận thông báo "Sách vừa hết" và được gợi ý Đặt giữ sách
...
**Given**: Sách đang có người khác Đặt giữ sách (reservation)
...
### **US-09: Đặt giữ sách sách (Reservation)**

#### **User Story Statement**

Là một Độc giả, tôi muốn Đặt giữ sách sách khi sách đang hết để nhận thông báo khi sách có sẵn và được ưu tiên mượn.

#### **Luồng Người dùng (User Flow)**

**Luồng chính: Đặt giữ sách sách**

1\. Customer xem chi tiết sách, thấy tình trạng "Hết hàng"

2\. Click "Đặt giữ sách"
...
**AC-01: Đặt giữ sách thành công**

**Given**: Sách đang hết
**When**: Customer click "Đặt giữ sách"
**Then**:

- Bản ghi reservation được tạo
- Hiển thị thông báo "Đặt giữ sách thành công. Bạn sẽ được thông báo khi sách có sẵn."
- Hiển thị vị trí trong hàng đợi

**AC-02: Thông báo khi sách có sẵn**

**Given**: Customer đã Đặt giữ sách và sách vừa được trả
...
**Given**: Customer đã hoàn tất mượn và trả sách (phiếu mượn ở trạng thái "Đã trả", "Đã trả — Đã thu phạt", hoặc "Đã trả — Đã hoàn cọc")
...
_(Tính năng này đã được gộp vào US-38 để tránh trùng lặp và nhất quán quy trình cho Guest)_
...
**Given**: Sách đang active, không có phiếu "Đang mượn" và không có yêu cầu Đặt giữ sách (Reservation) đang chờ
...
**Luồng ngoại lệ: Customer không đến lấy sách**

1\. Phiếu mượn ở trạng thái "Chờ lấy sách" quá thời hạn quy định (VD: 3 ngày)

2\. Hệ thống tự động hủy phiếu mượn

3\. Hoàn lại tồn kho chính thức và gửi email thông báo hủy cho Customer
...
\- Hành động (User, Book, BorrowTicket, ...)
...
Là Admin, tôi muốn cấu hình các hạng thành viên và quyền lợi tương ứng (giảm tỷ lệ tiền cọc, ưu tiên đặt giữ, tăng số sách mượn đồng thời) để khuyến khích độc giả sử dụng hệ thống.
...
\- Giảm tỷ lệ tiền cọc (VD: miễn cọc hoặc giảm 50% cọc)

\- Ưu tiên Đặt giữ sách
...
**Luồng phụ: Độc giả chưa có tài khoản (vãng lai / Guest Profile)**

1\. Staff tìm kiếm bằng SĐT

2\. Hệ thống kiểm tra:

\- Nếu không tìm thấy: hiển thị nút "Tạo hồ sơ độc giả vãng lai". Staff nhập Họ tên, SĐT, CMND/CCCD để tạo mới.

\- Nếu tìm thấy Guest Profile (đã tạo khi đặt mượn online ở US-37): hiển thị thông tin Guest. Hệ thống yêu cầu Staff bổ sung CMND/CCCD vào profile để chuyển thành hồ sơ hoàn chỉnh.

3\. Hệ thống lưu hồ sơ độc giả vãng lai (role: WALK_IN_CUSTOMER) với trạng thái đặc biệt.

4\. Staff tiếp tục quy trình tạo phiếu mượn cho độc giả vãng lai.
...
\- Sách còn trong kho khả dụng (Tổng tồn kho - Số lượng đang được đặt giữ > 0)
...
3\. Gợi ý "Đặt giữ sách cho độc giả?" nếu độc giả muốn đợi
...
3\. Hệ thống kiểm tra phương thức cọc ban đầu:

\- Nếu cọc bằng Tiền mặt: hiển thị "Hoàn cọc bằng tiền mặt: X VND". Staff hoàn tiền cho độc giả và click "Xác nhận đã hoàn cọc".

\- Nếu cọc bằng Ví điện tử: hiển thị "Tự động hoàn cọc về Ví điện tử: X VND". Hệ thống tự động tạo lệnh Refund qua cổng thanh toán.

4\. Hệ thống ghi nhận hoàn cọc, tạo biên nhận
...
- Nếu cọc bằng tiền mặt: Staff hoàn tiền mặt và xác nhận. Cọc ghi nhận "Đã hoàn".
- Nếu cọc bằng Ví điện tử: Hệ thống tự động tạo lệnh Refund qua cổng thanh toán.
...
**Luồng chính: Phê duyệt phiếu mượn Guest Online**

1\. Staff truy cập "Quản lý phiếu mượn"

2\. Chọn phiếu mượn Guest ở trạng thái "Chờ duyệt" và click "Phê duyệt"

3\. Hệ thống chuyển trạng thái sang "Chờ lấy sách" và gửi email thông báo cho Guest
...
\- Chuyển trạng thái phiếu từ "Chờ lấy sách" → "Đang mượn"
...
**Given**: Phiếu mượn Guest ở trạng thái "Chờ lấy sách", Guest đã đến thư viện
...
**AC-03: Phê duyệt phiếu mượn Guest Online**
...
- Tồn kho chính thức được trừ (hoặc chuyển từ tạm giữ sang giữ)
- Email thông báo sách đã sẵn sàng gửi cho Guest

**AC-04: Từ chối phiếu mượn Guest**
...
| US-09     | Đặt giữ sách sách (Reservation)                                    | Customer                   | Trung bình     |
